...
import config  # Certifique-se que config.N_CLASSES está definido

def dice_coef(y_true, y_pred, smooth=1e-7):
    """
    Dice multiclasse com one-hot automático em y_true.
    y_true: (batch, H, W)
    y_pred: (batch, H, W, N_CLASSES)
    """
    # One-hot em y_true
    y_true = tf.cast(tf.one_hot(tf.cast(y_true, tf.int32), depth=config.N_CLASSES), tf.float32)
